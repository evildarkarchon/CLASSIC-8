...
public class ScanResult
{
    ...
    public string GenerateTextSummary()
    {
        ...
        if (Errors.Count > 0)
        {
            ...
            foreach (var error in Errors.Take(10)) summary.AppendLine($"  - {error}");
            if (Errors.Count > 10)
        ...
        if (Warnings.Count > 0)
        {
            ...
            foreach (var warning in Warnings.Take(5)) summary.AppendLine($"  - {warning}");
            if (Warnings.Count > 5)
...
public class PerformanceMetrics
{
    ...
    public void DetectBottlenecks()
    {
        ...
        // CPU bottleneck
        if (AverageCpuUsage > 85) DetectedBottlenecks.Add("CPU");

        // Memory bottleneck
        if (CurrentMemoryUsage > PeakMemoryUsage * 0.9) DetectedBottlenecks.Add("Memory");

        // Cache bottleneck
        if (CacheHitRate < 70) DetectedBottlenecks.Add("Cache");

        // Worker thread bottleneck
        if (WorkerEfficiency < 60) DetectedBottlenecks.Add("Worker Threads");
