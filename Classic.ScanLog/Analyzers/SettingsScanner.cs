...
public class SettingsScanner : ISettingsScanner
{
    ...
    /// </summary>
    private void ValidateAchievementsSettings(Dictionary<string, object> settings, HashSet<string> xseModules, List<SettingValidationIssue> issues)
    {
        var achievementsEnabled = GetBooleanSetting(settings, "Achievements", false);
        var hasAchievementsMod = xseModules.Contains("achievements.dll") || xseModules.Contains("unlimitedsurvivalmode.dll");
        ...
        if (achievementsEnabled && hasAchievementsMod)
        {
            issues.Add(new SettingValidationIssue
            {
                ...
                Description = "Achievements Mod and/or Unlimited Survival Mode is installed, but Achievements parameter is set to TRUE",
                CurrentValue = "TRUE",
                ExpectedValue = "FALSE",
                SeverityScore = 4, // High severity
                FixInstructions = $"Open {_crashGenName}'s TOML file and change Achievements to FALSE, this prevents conflicts with {_crashGenName}."
            });
        }
    }
    ...
    /// </summary>
    private void ValidateMemoryManagementSettings(Dictionary<string, object> settings, HashSet<string> xseModules, List<SettingValidationIssue> issues)
    {
        ...
        if (memoryManagerEnabled && hasXCell)
        {
            issues.Add(new SettingValidationIssue
            {
                ...
                ExpectedValue = "FALSE",
                SeverityScore = 4, // High severity
                FixInstructions = $"Open {_crashGenName}'s TOML file and change MemoryManager to FALSE, this prevents conflicts with X-Cell."
            });
        }
            ...
            foreach (var (settingKey, displayName) in memorySettings)
            {
                if (GetBooleanSetting(settings, settingKey, false))
                {
                    issues.Add(new SettingValidationIssue
                    {
                        ...
                        ExpectedValue = "FALSE",
                        SeverityScore = 4, // High severity
                        FixInstructions = $"Open {_crashGenName}'s TOML file and change {settingKey} to FALSE, this prevents conflicts with X-Cell."
                    });
                }
            }
        }
    ...
    /// </summary>
    private void ValidateLooksMenuSettings(Dictionary<string, object> settings, HashSet<string> xseModules, List<SettingValidationIssue> issues)
    {
        ...
        if (!f4eeEnabled && hasLooksMenu)
        {
            issues.Add(new SettingValidationIssue
            {
                ...
                ExpectedValue = "TRUE",
                SeverityScore = 4, // High severity
                FixInstructions = $"Open {_crashGenName}'s TOML file and change F4EE to TRUE, this prevents bugs and crashes from Looks Menu."
            });
        }
    }
