...
public class SqliteConnectionPool : IDisposable
{
    ...
    /// </summary>
    public async Task<PooledConnection> GetConnectionAsync(string databasePath,
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var poolEntry = _connectionPools.GetOrAdd(databasePath,
                path => new ConnectionPoolEntry(path, _maxConnectionsPerDatabase));
    ...
    private void CleanupExpiredConnections(object? state)
    {
        ...
        foreach (var kvp in _connectionPools)
        {
            ...
            if (kvp.Value.IsEmpty) expiredPools.Add(kvp.Key);
        }
    ...
    public void Dispose()
    {
        if (!_disposed)
        {
            ...
            foreach (var poolEntry in _connectionPools.Values) poolEntry.Dispose();
...
internal class ConnectionPoolEntry : IDisposable
{
    ...
    public int CleanupExpiredConnections(TimeSpan timeout)
    {
        ...
        foreach (var entry in remainingConnections) _availableConnections.Enqueue(entry);
    ...
    private async Task<SQLiteConnection> CreateNewConnectionAsync(CancellationToken cancellationToken)
    {
        var connectionString =
            $"Data Source={_databasePath};Version=3;Cache Size=10000;Page Size=4096;Temp Store=Memory;";
    ...
    public void Dispose()
    {
        if (!_disposed)
        {
            ...
            while (_availableConnections.TryDequeue(out var entry)) entry.Connection.Dispose();

            foreach (var connection in _activeConnections.Keys) connection.Dispose();
