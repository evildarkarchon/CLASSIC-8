    ...
    public async Task RestoreWindowStateAsync(Window window)
    {
        try
        {
            ...
            if (settings.X.HasValue && settings.Y.HasValue)
            {
                ...
                if (IsPositionValid(position)) window.Position = position;
            }
            ...
            if (Enum.TryParse<WindowState>(settings.WindowState, out var windowState)) window.WindowState = windowState;
