...
public sealed class YamlSettingsCache : IYamlSettingsCache
{
    ...
    public T? GetSetting<T>(YamlStore yamlStore, string keyPath, T? newValue = default)
    {
        ...
        if (newValue == null && StaticYamlStores.Contains(yamlStore) &&
            _settingsCache.TryGetValue(cacheKey, out var cachedValue)) return (T?)cachedValue;
        ...
        try
        {
            ...
            if (current is IDictionary<object, object> readContainer &&
                readContainer.TryGetValue(lastKey, out var value))
            {
                var result = ConvertValue<T>(value);

                // Cache the result for static stores
                if (StaticYamlStores.Contains(yamlStore)) _settingsCache.TryAdd(cacheKey, result);
    ...
    {
        if (!File.Exists(yamlPath)) return new Dictionary<object, object>();
    ...
    private static T? ConvertValue<T>(object? value)
    {
        ...
        {
            // Direct type match
            if (value.GetType() == underlyingType) return (T)value;

            // Handle string to Path conversion
            if (underlyingType == typeof(FileInfo) && value is string strPath) return (T)(object)new FileInfo(strPath);
